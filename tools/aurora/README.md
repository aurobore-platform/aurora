...
| npm / node | Действие |
|---|---|
| `pnpm demos:verify` | Web + `aurobore build` для всех `examples/*` (11 demos, fail-fast); скрипт `verify-demos.mjs` |
| `pnpm e2e:verify` | E2E harness: `hello-world-stub` → `aurobore build` → `aurobore run` с bridge-assert (`AUROBORE_E2E=1`); нужен эмулятор |
...
| `pnpm container:sync` | sync `runtime/container` из repo (включая `qml/verification/`) + extraSync (bridge-native, native-sdk, plugins). В отличие от `aurobore build`, harness не исключается — нужен для `AUROBORE_W*` тестов |
