...
> Формат — [ADR-006](https://github.com/aurobore-platform/aurora/blob/main/docs/adr/ADR-006-configuration-format.md).
...
Только **нативный** QML (splash, cover): строки `qsTr("AUROBORE_APP_NAME")` → `.ts` / `.qm` через Qt Linguist.
Web-UI локализуется средствами фронтенда (vue-i18n и т.д.), не через этот каталог.
