...
> **СТАТУС ДЛЯ АГЕНТОВ: ✅ ЗАВЕРШЁН.** Все майлстоуны **M0–M5** закрыты, чек-боксы `[x]`.
> Sign-off MVP: **2026-06-28** (`pnpm container:all` на SDK 5.2.1.200, M1/M2/M3 OK). Новых задач по MVP нет —
> файл держим как исторический référence. Активная работа — см. [alpha-plan.md](alpha-plan.md) и
> [roadmap.md](roadmap.md) (этап Beta).
...
- [x] Asset Loader: loopback HTTPS origin (`https://127.0.0.1:<port>/`) через `AssetSchemeServer` +
      ...
      (прямой CEF scheme handler недоступен в public SDK, V-13; см. [runtime/container/README.md](../runtime/container/README.md)).
