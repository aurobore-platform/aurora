...
| V-16 | 🟢 **Закрыт (A2, эмулятор).** Fullscreen WebView, native→CSS vars (`Theme.statusBarHeight`, `SafeZoneRect`), `aurobore-chrome.css`, runtime viewport normalize, `systemChrome:insetsChanged`. Клавиатура **overlay-only** (`KeyboardInput` off, `navigator.virtualKeyboard.overlaysContent`); bottom inset: **visualViewport → native** `injectInsets` (без resize WebView / `innerHeight` jump). Journal: `A2 chrome OK`, `A2 keyboard OK`, `A2 OK`. Физическое устройство — рекомендуется для финального sign-off. | 🟢 | Средний (UX) | `pnpm container:all` |
