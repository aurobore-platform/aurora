...
| V-16 | Системные insets / status bar: zero-config layout (native → CSS vars + `aurobore-chrome.css`); контент не перекрывается status bar; rotation; клавиатура | ⏳ | Средний (UX) | A2: `pnpm container:all` на эмуляторе + устройство; vanilla без safe-area CSS |
