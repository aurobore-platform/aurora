...
> Ведётся вместе с [sources.md](https://github.com/aurobore-platform/aurora/blob/main/docs/aurora/sources.md); связан с ADR-002/004/007 и `requirements.md` (§5, A1–A5).
...
**Действие:** соответствующие пометки `(verify)` в [ADR-004](https://github.com/aurobore-platform/aurora/blob/main/docs/adr/ADR-004-webview-engine-abstraction.md),
[ADR-007](https://github.com/aurobore-platform/aurora/blob/main/docs/adr/ADR-007-packaging-build.md) и `requirements.md` (A2) можно понизить до «подтверждено,
...
| V-cover | 🟡 **Реализовано (A2+UI Kit).** `CoverTemplate` + иконка приложения; `cover.actions` (до 4); `cover.mode: preview` (grab WebView); API `Cover` + `?coverDemo=1`. Ручная проверка tap на обложке — pending. | 🟡 | Низкий (UX) | `pnpm container:all`; `?coverDemo=1`; `cover.mode: preview` в config |
| V-uikit | 🟡 **Реализовано (UI Kit native chrome).** `.desktop`: `X-Nemo-Application-Type`, `[X-Aurora-SplashScreen]`, `[X-Aurora-Application]` (orientation, IconMode); in-app `SplashScreen.qml` (gradient + icon); doctor `inspectProjectIcons`. См. [ui-kit.md](ui-kit.md). Ручной sign-off на эмуляторе — pending. | 🟡 | Низкий (store UX) | `pnpm test`; `aurobore doctor`; `pnpm container:all` |
...
| V-webview-cookies | 🟡 **Реализовано (W5).** Public `CookieManager` (`aurorawebview/cookies/cookiemanager.h`): `clearCookies` native; **`setCookie`: interim until native API** — QML orchestration (`document.cookie` после navigate на whitelisted domain; SDK 5.2.1 без public `setCookie`, в отличие от OMP Flutter internal API). Bridge plugin `webview` (`setCookie` / `clearCookies`). Sign-off — `AUROBORE_W5_COOKIES=1` на эмуляторе. Journal: `W5 cookie test: setCookie OK`, `W5 cookie OK: Cookie header verified`, `W5 cookie OK: cleared`. См. [webview.md](webview.md) §6.3 Interim, `V-webview-setcookie-interim`. | 🟡 | Средний (гибридные apps) | `AUROBORE_W5_COOKIES=1 pnpm container:run`; см. [webview.md](webview.md) §6.3 |
| V-webview-dispose | 🟡 **Реализовано (W6).** `Loader` + `teardownWebView` / `recreateWebView` в [`WebAppPage.qml`](https://github.com/aurobore-platform/aurora/blob/main/runtime/container/qml/pages/WebAppPage.qml) (lifecycle); WebView instance — [`AuroboreWebView.qml`](https://github.com/aurobore-platform/aurora/blob/main/runtime/container/qml/components/AuroboreWebView.qml). `aboutToQuit`: `AssetSchemeServer::stop()`, `WebEngineContext::Shutdown()`. Harness: `AUROBORE_W6_DISPOSE=1` — 10 циклов после M3 OK. Journal: `W6 dispose cycle N/10`, `W6 OK: 10 dispose cycles complete`. Sign-off — `ps \| grep webview-subprocess` без роста на эмуляторе. | 🟡 | Низкий (dev reload) | `AUROBORE_W6_DISPOSE=1 pnpm container:run`; см. [webview.md](webview.md) §6.4 |
| V-webview-harness-split | 🟢 **Закрыт (W+1).** Harness W3–W6 вынесен в `runtime/container/qml/verification/`; `WebAppPage.qml` без test-only кода. `generateNativeProject` исключает `qml/verification/` из `.aurobore/native` (`SYNC_EXCLUDE_PATHS`); dev-toolkit (`pnpm container:sync`) sync из repo с harness. | 🟢 | Низкий (prod packaging) | `pnpm test` (generate.test.ts); `AUROBORE_W4_AUTH=1 pnpm container:run`; см. [webview-improvements-plan-plus.md](https://github.com/aurobore-platform/aurora/blob/main/docs/dev/webview-improvements-plan-plus.md) W+1.4 |
| V-webview-allowedOrigins-config | 🟢 **Закрыт (W+2).** `web.allowedOrigins` в `aurobore.config.json` → schema/parse → `generateDefaultsJson` → `config/defaults.json` → `AppConfig::allowedOrigins()` → QML URL policy. Валидация: только `https://` origin без path; непустой whitelist требует `Internet`. Gate: `examples/hybrid-demo`. | 🟢 | Средний (гибридные apps) | `pnpm test`; `pnpm demos:verify` (hybrid-demo); `aurobore run` на эмуляторе → tap external link |
| V-webview-setcookie-interim | 🟢 **Закрыт (W+3).** W5 `setCookie` задокументирован как interim until native API. Interim: QML orchestration; target: `CookieManager::setCookie` при появлении в public SDK; bridge API стабилен. | 🟢 | Низкий (гибридные apps) | См. [webview.md](webview.md) §6.3 Interim; комментарии в `WebViewCookieBridge`, `WebViewCookieOrchestrator.qml` |
| V-webview-qml-decompose | 🟢 **Закрыт (W+4).** `WebAppPage.qml` ≤350 строк (оркестратор); WebView + URL policy + chrome signals — [`AuroboreWebView.qml`](https://github.com/aurobore-platform/aurora/blob/main/runtime/container/qml/components/AuroboreWebView.qml); cutout/inject — [`WebChrome.js`](https://github.com/aurobore-platform/aurora/blob/main/runtime/container/qml/logic/WebChrome.js); bridge messages — [`BridgeMessages.js`](https://github.com/aurobore-platform/aurora/blob/main/runtime/container/qml/logic/BridgeMessages.js). | 🟢 | Низкий (maintainability) | `pnpm container:all`; `grep LoadRequestExtension runtime/container/qml/pages/` → пусто |
| V-webview-getusermedia | ⏳ **Открыт (спайк для FR-S6 / [ADR-011](https://github.com/aurobore-platform/aurora/blob/main/docs/adr/ADR-011-camera-frame-channel.md)).** Стратегия getUserMedia — **engine-first, plugin-fallback**. Проверить на **устройстве с камерой** (эмулятор x86 → `CAMERA_UNAVAILABLE`, V-camera), на странице **без полифила**: (1) есть ли `navigator.mediaDevices.getUserMedia` в движке CEF; (2) поток/reject и код ошибки; (3) можно ли выдать media-permission из натива (обёртка `ru.auroraos.WebView` / CEF media-access handler в `runtime/container`). Резолюция определяет, нужен ли Camera-fallback (`watchPreview` + бинарные кадры) вообще. `mediaDevices`-полифил — opt-in, не в дефолте. | ⏳ | Средний (SPA камера / гибриды) | Устройство с камерой; страница без полифила: `navigator.mediaDevices.getUserMedia({video:true})`; журнал permission |
